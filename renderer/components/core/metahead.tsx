...
export declare interface MetaHeadProps {
    title: string;
    desc: string;
    url?: string | null | undefined;
}

export default function MetaHead({ title, desc, url = null }: MetaHeadProps): ReactElement<MetaHeadProps> {
    const router: NextRouter = useRouter();
    const hasWindow: boolean = window !== undefined;
    const protocol: string = hasWindow && location.protocol ? location.protocol : '';
    const hostname: string = hasWindow && location.hostname ? location.hostname : '';
    // const origin: string = hasWindow && location.origin ? location.origin : '';
    const host_url: string = `${protocol}//${hostname}`;

    // const svg: string = `${host_url}/api/svg?header=${title}&caption=${desc}`;
    const svg: string = `/logo.webp`;
