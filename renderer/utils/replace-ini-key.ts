...
//
export default function replaceIniKey(
    iniContentString: string,
    section: string,
    key: string,
    newValue: string | number | boolean | bigint
): string {
    // Create a regular expression to match the specific section and key
    const sectionRegex: RegExp = new RegExp(`\\[${section}\\][\\s\\S]*?${key}\\s*=\\s*.*`, 'g');
     ...
     * @returns {string}
     */
    const replacer = (s: string): string => s.replace(new RegExp(`(${key}\\s*=\\s*)[^\\n]*`), `$1${newValue}`);
